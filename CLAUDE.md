...
## TODO
- **Structural outbound redesign — decision pending (session 2026-04-18e brainstorm).** Three options on the table: (A) Cloud API + Utility templates for punctual reminders + Whapi for reactive — preserves product promise, costs ~$30/mo, requires positioning shift to "family task utility"; (B) Boti formula — 100% reactive on Whapi, no punctual reminders, no queue drain, no recovery, ever — safest but loses punctual-reminder promise; (C) device-side automation hybrid. Yaron deciding in fresh session. Meta business verification already submitted (blocked on phone registration due to active 24h restriction). Do NOT attempt to register bot phone 55-517-5553 on Cloud API during restriction window.
