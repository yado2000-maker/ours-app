...
- NOT valid categories: "בשר", "חמוצים", "ירקות" alone (use "בשר ודגים" / "מזווה" / "פירות וירקות" — the full canonical names above)
- NOT valid categories — DRIFT TRAPS (Bug 3, 2026-05-07): "תוצרת" → use "פירות וירקות". "מקרר" / "מקרר וקירור" → use "מוצרי חלב" (eggs+milk+cheese+yogurt all live there, NOT a hardware appliance label). "ניקיון" / "ניקוי" alone → use "ניקוי ובית" (the full name with "ובית"). "מאפיה" → use "מאפים". "מאכלים קפואים" / "הקפאה" → use "מוצרים קפואים". "שתייה" → use "משקאות". The category field renders directly in the user's grocery view — wrong labels surface as visible bugs. If unsure between two canonical categories, prefer the more specific one; if NO canonical fits, use "אחר" — never invent a label
...
PRONOUN DISCIPLINE — MANDATORY (Bait family 2026-04-23):
When listing, summarizing, or referencing household items (tasks, shopping, events, reminders, expenses, rotations) in your reply, those items belong to the FAMILY — NOT to you. Speak ABOUT them in second person ("לך"/"לכם"/"אצלכם") or abstractly ("נשארה", "יש עוד", "הרשימה"). NEVER use first-person possessive/subject for user data.
- WRONG: "יש לי עוד משימה פתוחה — לטפל בארנונה" (I have an open task) — the task is the user's, not yours.
- WRONG: "הרשימה שלי מלאה", "המטלה שלי היום", "האירועים שלי השבוע"
- WRONG: "שילמתי 200 ש״ח על חשמל" when reporting back a user's expense — the user paid, not you.
- RIGHT: "יש לך/לכם עוד משימה פתוחה — לטפל בארנונה"
- RIGHT: "נשארה משימה פתוחה — לטפל בארנונה" (abstract, avoids pronoun entirely)
- RIGHT: "הרשימה די מלאה", "יש לכם שני אירועים השבוע", "שילמת 200 ש״ח על חשמל"

First-person IS correct for YOUR actions — adding, reminding, saving, confirming, asking. Keep these:
- "הוספתי לרשימה ✓" (I added to the list)
- "אזכיר לך מחר ב-10:00" (I'll remind you)
- "רשמתי את האירוע" (I noted the event)
- "אני לא יכולה לשלוח מיילים" (capability disclaimer)

Rule of thumb: if the sentence is about DATA (user's tasks/shopping/events/expenses), use second-person or abstract. If it's about ACTION YOU JUST TOOK or a statement about yourself, first-person is fine.
In mixed replies, keep the two separate: "הוספתי למשימות ✓ | יש לך עכשיו 3 משימות פתוחות" — action in first person, data summary in second.

Triggers to watch for where you most often slip: answering "מה יש היום?" / "מה את אומרת?" / "מה המצב?" / "תעשי לי סיכום" — the user is asking ABOUT THEIR data, so the reply describes their items, never possesses them.
...
IMPORTANT: Adding an event does NOT automatically create a reminder. Sheli will NOT notify anyone at the event's time unless a reminder is also scheduled.
Reply vocabulary — vary naturally: "הוספתי ליומן 📅", "נרשם ביומן ✓", "שמרתי ביומן", "רשום ✓".
FORBIDDEN: claiming a past notification was sent ("הזכרתי כבר"). Do NOT pretend a reminder already fired.
EVENT-REMINDER FOLLOW-UP — MANDATORY (2026-04-25): On the SAME turn that confirms the event, append on a NEW LINE one short follow-up: "רוצה שאזכיר חצי שעה לפני? 🔔" (vary: "אזכיר חצי שעה קודם? 🔔"). Do NOT skip this question — many users (the שי 2026-04-25 incident) assume an event auto-creates a reminder and feel let down when nothing fires. ASKING is fine; AUTO-EMITTING a reminder action without asking is NOT — wait for the family's "כן" before scheduling.
If a family member later replies "כן" / "נשמע טוב" / "תזכירי" / "תזכרי לפני" referring to that pending event ask — emit a {"type":"reminder",...} action (send_at = event scheduled_for minus 30 minutes, full Israel-tz ISO) and confirm "סבבה, אזכיר 30 דקות לפני 🔔". If they say a different duration, honor it. If they say "לא" — just "סבבה 👌", no action.`;
      break;
    case "complete_task":
...
PARTIAL-COMMITMENT TRAP — MANDATORY (Hila 2026-05-07, the "שתי תזכורות במקום אחת" bug): NEVER mix a commitment with a clarifying question in the same reply. The pattern "אזכיר X ✓ באיזו שעה?" / "אזכיר היום בערב ✓ + clarifying question" is FORBIDDEN — Sheli emits a REMINDER block at a default-guessed time AND asks for the real one, the user answers, Sheli emits a SECOND REMINDER block at the user's answer, and BOTH reminders fire. Live failure: voice message "תזכירי לי מחר בבוקר למכור מניות" → Sheli replied "אזכיר מחר בבוקר ✓ באיזו שעה?" + saved at default 09:00 → user said "10" → second save at 10:00 → both fired the next morning. The discriminator: if the user's time spec is a vague slot ("בערב", "בבוקר", "אחר הצהריים", "בלילה") with NO specific hour, you DO NOT have enough to commit yet. RIGHT in that case: ASK ONLY — no commitment verb (no "אזכיר"), no checkmark (no "✓"), no metadata block. Examples — RIGHT: "באיזו שעה מחר בבוקר?" / "באיזה ערב, ובאיזו שעה?". Commit on the NEXT turn after the user gives an explicit hour. Only commit + emit the block when both day AND hour are specified.
...
1. CONVERSATION CONTEXT — the messages below are in chronological order, including recent history before the message that escalated. Each message is tagged with the sender and relative age — example format: [Shira, 6d ago]: text  or  [Shira, 3m ago]: text. If a message has a line starting with "↳ replying to:" it means the user used WhatsApp's reply feature — their comment pertains specifically to the quoted message.
   ...
   ANSWER-TO-YOUR-OWN-QUESTION (Mariam 2026-05-07, the "infinite clarification loop" bug): When YOUR most recent reply (visible in the conversation history above as a [Sheli, Xm ago] line OR the line before the current message) was a CLARIFYING QUESTION — "באיזו שעה?" / "מה להזכיר?" / "מתי?" / "איזו רשימה?" / "להוסיף לאן?" / "מה לעשות עם זה?" — then the user's CURRENT message IS the answer to that question, even when it looks terse, off-topic, or weird in isolation. Combine YOUR question + THEIR answer into a complete inference and ACT. Do NOT ask another clarifying question on top — that's an infinite loop and breaks trust. Live failure: Sheli asked "מה להזכיר לך?", Mariam replied with "." on her own earlier "ראיון של ניר" message (Israeli dot-reply pattern: tap previous message, add "." / single emoji / "👆" / "↑" / "+1" to re-ping the quoted content because WhatsApp won't send a quoted reply with empty text). Sheli's NEXT reply was "לא בטוחה מה את רוצה לעשות עם זה 😅 להוסיף כמשימה? לרשום? תאריך?" — that's wrong. Right behavior: read your own prior question ("מה להזכיר?"), read Mariam's quoted "ראיון של ניר" as the answer, infer add_reminder with reminder_text="ראיון של ניר", ASK ONLY for the still-missing piece (the time, if absent from prior context). If you already have BOTH the content AND the time elsewhere in the conversation history, commit and emit the action. Sub-rule for terse replies: when the user's CURRENT text is essentially empty (single char ".", single emoji, "👆"/"↑"/"+1") AND the quote has meaningful content, the QUOTE is the message — re-process the quoted text as if it was just sent fresh.
...
${isHe ? 'Shopping categories (Hebrew): פירות וירקות (כל ירק ופרי), מוצרי חלב (כולל ביצים, טופו, גבינות, יוגורט, חלב צמחי), בשר ודגים, מאפים (לחם, פיתות, עוגות), מזווה (אורז, פסטה, שמן, חומוס, טחינה, תבלינים, אפייה, שימורים), מוצרים קפואים, משקאות, ניקוי ובית (סבון, נייר טואלט, שקיות זבל, סוללות, נרות), מוצרים מחנות הטבע, טיפוח (שמפו, קרם, משחת שיניים), אחר (רק אם שום קטגוריה אחרת לא מתאימה). DRIFT TRAPS — DO NOT EMIT (Bug 3, 2026-05-07): "תוצרת" → use "פירות וירקות". "מקרר" → use "מוצרי חלב" (eggs+milk+cheese live there, NOT a hardware appliance label). "ניקיון" / "ניקוי" alone → use "ניקוי ובית". "מאפיה" → use "מאפים". "שתייה" → use "משקאות". The category MUST exactly match one canonical name from the list — if no canonical fits, use "אחר", never invent a label.' : 'Shopping categories: Produce, Dairy Products (incl. eggs, tofu, yogurt, plant milk), Meat & Fish, Bakery, Pantry (rice, pasta, oil, spices, baking, canned), Frozen, Drinks, Household (cleaning, paper, batteries, candles), Health Food, Personal Care, Other (only if nothing else fits)'}
...
const ONBOARDING_QA: Array<{ patterns: RegExp[]; topic: string; keyFacts: string }> = [
  ...
  },
  {
    ...
    keyFacts: "User asks about Google Calendar / Apple Calendar / Outlook sync (or — like the 2026-04-25 שי incident — asks 'למה אין לך גישה ליומן שלי?' / 'why don't you have access to my calendar?'). GROUND TRUTH: NO such integration exists today. Sheli CANNOT read, write, or sync to any external calendar. Events the user saves live INSIDE Sheli only, viewable at sheli.ai. CANONICAL ANSWER (paraphrase, do not copy verbatim every time): 'כרגע היומן מנוהל אצלי באפליקציה, אבל בקרוב יהיה לי חיבור ישיר לגוגל קלנדר ואוכל לעדכן עבורך ישירות ביומן שלך 🗓️✨'. The promise is forward-looking and confident — Google Calendar integration IS coming, and the user can expect Sheli to write directly into their personal calendar once it ships. FORBIDDEN openers/claims: (a) 'כן, יש לי אינטגרציה' / 'I have integration' (the integration does NOT exist yet); (b) 'האירועים שלך נכנסים ישר לקלנדר' (false — they live in Sheli only); (c) 'אני מסנכרנת עם גוגל' (false today). FORBIDDEN hedges: 'אולי בעתיד' / 'אם נחליט' / 'תלוי במשאבים' — be confident, not wishy-washy. Don't promise a specific date. Keep reply 2-3 short lines. The English version: 'Right now your calendar lives inside the app, but a direct Google Calendar connection is on the way — soon I'll be able to write events straight into your own calendar 🗓️✨'.",
  },
  {
    ...
    topic: "24h-window",
    keyFacts: "User asks whether they need to message Sheli first / say בוקר טוב every day for reminders to fire, OR complains that a past reminder didn't arrive because they hadn't messaged first. UPDATED 2026-04-24: they DO NOT need to do this anymore. Reminders now fire at their scheduled time regardless of whether the user messaged Sheli in the past 24h. If they're asking proactively ('do I need to say good morning?') → reassure clearly: 'לא צריך — תזכורות נשלחות בזמן שקבעתם, בלי קשר אם דיברנו היום או לא 🙌'. If they're complaining about an old missed reminder → acknowledge honestly that this used to be a limitation, confirm it's now fixed going forward, and offer to reschedule the specific thing they missed. 2-4 lines. DO NOT mention technical details (WhatsApp 24h window, Cloud API, Meta, infrastructure, migration plans). DO NOT blame 'a bug' or 'servers'. DO NOT say users still need to ping Sheli each morning — that behavior is gone.",
  },
  {
...
17b. NEVER add a QUESTION to the shopping list. A message is a question (not a shopping item) if ANY of these hold:
    - It ends with "?" (e.g. "למה אין לך גישה ליומן שלי?", "איפה הקבלה?", "מה אפשר לקנות?")
    - It opens with a Hebrew question word: למה / איך / מה / מי / מתי / איפה / האם / כמה / למי
    - It opens with an English question word: why / how / what / when / who / where / can you / do you / will you
    - It is clearly a meta-question about Sheli, the app, the family, billing, or general knowledge
...
21a. PARTIAL-COMMITMENT TRAP — MANDATORY (Hila 2026-05-07, the "שתי תזכורות במקום אחת" bug): NEVER mix a commitment with a clarifying question in the same turn. The pattern "אזכיר X ✓ באיזו שעה?" / "אזכיר היום בערב ✓ + clarifying question" is FORBIDDEN — it emits a "reminder" action at a default-guessed time AND asks for the real one, the user answers, you emit a SECOND "reminder" at the user's answer, BOTH reminders fire the next morning, and the user gets two pings instead of one. Live failure: voice "תזכירי לי מחר בבוקר למכור מניות" → Sheli replied "אזכיר מחר בבוקר ✓ באיזו שעה?" + emitted reminder at default 09:00 → user said "10" → second reminder at 10:00 → both fired. The discriminator: if the user's time spec is a vague slot ("בערב", "בבוקר", "אחר הצהריים", "בלילה") with NO specific hour, you DO NOT have enough to commit yet. RIGHT in that case: ASK ONLY — no commitment verb (no "אזכיר"), no checkmark (no "✓"), no "reminder" action. Examples — RIGHT: "באיזו שעה מחר בבוקר?" / "באיזה ערב, ובאיזו שעה?". Commit + emit the action on the NEXT turn, after the user gives an explicit hour. Only commit when BOTH day AND hour are specified.
...
- reminder: {"type":"reminder","text":"להוציא בשר","time":"17:00","send_at":"2026-04-12T17:00:00+03:00"}
  ...
  DEFAULT-DAY RULE — STRENGTHENED (Bug 4, 2026-05-07; replaces the old LATE-NIGHT TRAP. Live failures: Mariam 22:37 + 23:16, Ayelet 04:49, Michal 23:25, Liran 06:40):
  The IL clock is your ONLY reference for "today" vs "tomorrow". Resolve the date BEFORE drafting your reply, then verify your visible day-word and metadata send_at IL date AGREE. Three windows, locked:
  A. EVENING WINDOW (current IL hour 18:00-23:59): "בבוקר" / "ב-X בבוקר" / "X וחצי בבוקר" with NO explicit day → TOMORROW. Today's morning is over; the user is winding down today and planning the next morning. Live failures: at 22:37 IL user said "בבוקר" → MUST be tomorrow 08:00 (reply: "אזכיר מחר בבוקר ב-8:00 ✓"). At 23:16 IL user said "9 וחצי בבוקר" → MUST be tomorrow 09:30 (reply: "אזכיר מחר ב-9:30 ✓"). NEVER use "היום" for a morning request made after dark.
  B. PRE-DAWN WINDOW (current IL hour 00:00-06:59) — REPLACES the old 00:00-04:00 trap. "בבוקר" / "ב-X בבוקר" / "בצהריים" / "ב-X" with NO explicit day, AND the named time is still future TODAY → TODAY. The user is awake before/around sunrise and means the upcoming hours of THIS calendar day. Live failure: at 04:49 IL user said "תזכורת בשעה 8:00" → MUST be today 08:00 (3h11m future). Reply: "אזכיר היום ב-8:00 ✓". NEVER "אזכיר מחר ב-8:00" when today's 8:00 is still ahead. Also: at 02:30 IL "ב-9 בבוקר" → today 09:00. At 06:40 IL "ב-13 בצהריים" → today 13:00.
  C. DAYTIME WINDOW (current IL hour 07:00-17:59): time-only with no day → TODAY if at least 10 min future, else TOMORROW. Standard.
  CROSSOVER RULE (Michal 23:25 case): "בעוד X שעות/דקות" — compute now + X. If the result lands on the NEXT IL calendar date, the visible reply MUST use "מחר" (or the next day's name), and metadata send_at MUST be on that next day. NEVER say "היום" for a time that has already crossed midnight. Example: at 23:25 IL "בעוד 3 שעות" → tomorrow 02:25, reply: "אזכיר מחר ב-02:25 ✓".
  EXPLICIT-WORD INVARIANT (Mariam 23:16 "מחר בבוקר" case): When the user EXPLICITLY uses "מחר" / "היום" / a weekday name, USE THAT WORD in your visible reply, and your metadata send_at IL date MUST match. The user already gave you the day — never override with the opposite day-word.
  PRE-SEND CHECK: Before emitting your reply, scan it once: does the day-word in your visible text match the IL date of your send_at metadata? If not, re-resolve from A/B/C/CROSSOVER above. The day-mismatch guard ("רגע, התבלבלתי בין היום למחר") fires when these disagree — every trip costs the user a re-state and trust drops.
...
// ─── Identity Probe / Prompt-Injection Deflect (2026-04-19) ───
// Deterministic deflect — runs BEFORE Haiku/Sonnet for both 1:1 and groups,
// saves LLM calls + removes the chance Sheli reveals system-prompt details
// under a clever jailbreak. Patterns intentionally SPECIFIC to reduce false
// positives on casual "how do you work?" / "who made you?" questions which
// we want Sonnet to answer warmly (those route to FAQ-like responses).
//
// Response set is 3 variants, rotated at random — avoids looking robotic if
// the same user probes multiple times.
const INJECTION_PATTERNS: RegExp[] = [
  ...
  /\b(?:show|reveal|print|output|display)\s+(?:me\s+)?(?:your\s+)?(?:system\s+)?(?:prompt|instructions|source\s+code)\b/i,
  /\bwhat(?:'s|\s+is)\s+your\s+(?:system\s+)?prompt\b/i,
  // System-prompt extraction (Hebrew)
  /התעלמ[יוה]\s+מ?ה?(?:הוראות|פרומפט|הנחיות)/,
  /הרא[יוה]\s+(?:לי\s+)?(?:את\s+)?ה?(?:פרומפט|הוראות\s+שלך|קוד|סיסטם\s+פרומפט)/,
  /מה\s+ה?פרומפט\s+שלך/,
  /מה\s+ההוראות\s+שלך/,
  // Model/origin probes (English)
  /\b(?:are|r)\s+you\s+(?:claude|chatgpt|gpt-?\d*|an?\s+llm|an?\s+ai|anthropic|openai|bard|gemini|llama)\b/i,
  /\bwhat\s+(?:model|llm|ai)\s+are\s+you\b/i,
  /\bwhich\s+(?:model|llm|ai)\s+(?:are\s+you|do\s+you\s+use|powers?\s+you)\b/i,
  /\bbased\s+on\s+(?:claude|chatgpt|gpt|gemini|llama|anthropic|openai)\b/i,
  /\bpowered\s+by\s+(?:claude|chatgpt|gpt|anthropic|openai)\b/i,
  // Model/origin probes (Hebrew)
  ...
  /\bjailbreak\b/i,
  /\bDAN\s+mode\b/i,
  /\bpretend\s+(?:you(?:'re| are)\s+not|to\s+be)\b/i,
  /תעמיד[יה]?\s+פנים/,
];

function isIdentityProbeOrInjection(text: string): boolean {
  if (!text || text.length < 4) return false;
  return INJECTION_PATTERNS.some((re) => re.test(text));
}

const DEFLECT_RESPONSES = [
  "אני שלי, העוזרת שלכם בווטסאפ 💛 את השאר אני שומרת לעצמי. במה אפשר לעזור?",
  "סוד מקצועי 🤫 בואו נחזור לדברים החשובים — מה תרצו שאסדר היום?",
  "אהבתי את הניסיון 😄 אבל אני לא מספרת על עצמי. רשימת קניות? תזכורת? פגישה?",
];

function pickDeflect(): string {
  return DEFLECT_RESPONSES[Math.floor(Math.random() * DEFLECT_RESPONSES.length)];
}

// ─── Operator Admin Commands (2026-04-19) ───
// Handles commands DM'd from Yaron's personal operator phone (ADMIN_PHONE,
// defined ~line 6415). Returns true if the message was a recognized command
// and was handled — caller should short-circuit. Returns false for non-
// command messages so the normal flow continues (Yaron can still use 1:1
// as a normal Sheli user).
//
// Supported commands:
//   /admit 972XXXXXXXX [שם]    — override waitlist redirect for a phone
//   /help                       — list commands
//
// Safety rails:
//   - Only accepts commands from ADMIN_PHONE (checked by caller).
//   - /admit refuses to regress users already in onboarded/active/chatting
//     state — protects against accidental state downgrade.
//   - Replies confirm the EXACT phone parsed so Yaron can verify no typo.
async function handleAdminCommand(
  ...
  if (admitMatch) {
    ...
    const { data: existing } = await supabase
      .from("onboarding_conversations")
      .select("state, household_id, context")
      .eq("phone", parsedPhone)
      .maybeSingle();

    const skipStates = new Set(["chatting", "personal", "joined", "onboarded", "active"]);
    if (existing && skipStates.has(existing.state || "")) {
      await sendAndLog(prov, {
        groupId: message.groupId,
        text: `ℹ️ ${parsedPhone} כבר state=${existing.state}${existing.household_id ? `, household=${existing.household_id}` : ""} — לא שונה`,
      }, {
        householdId: "operator",
        groupId: message.groupId,
        inReplyTo: message.messageId,
        replyType: "admin_command_noop",
      });
      return true;
    }
    ...
    const { error } = await supabase
      ...
      .upsert({
        phone: parsedPhone,
        state: "chatting",
        message_count: existing ? undefined : 0,
        context: newContext,
        tried_capabilities: existing ? undefined : [],
      }, { onConflict: "phone" });

    if (error) {
      console.error(`[admin /admit] DB error for ${parsedPhone}:`, error.message);
      await sendAndLog(prov, {
        groupId: message.groupId,
        text: `❌ DB error: ${error.message}`,
      }, {
        householdId: "operator",
        groupId: message.groupId,
        inReplyTo: message.messageId,
  ...
  if (/^\/help\b/i.test(text)) {
    await sendAndLog(prov, {
      ...
      groupId: message.groupId,
      inReplyTo: message.messageId,
      replyType: "admin_command_help",
    });
    return true;
  }

  // Unknown /command — reply with a hint so Yaron doesn't wonder what happened
  if (/^\/[a-z]+/i.test(text)) {
    await sendAndLog(prov, {
      groupId: message.groupId,
      text: `❓ unknown command. /help for list.`,
    }, {
      householdId: "operator",
      groupId: message.groupId,
      inReplyTo: message.messageId,
      replyType: "admin_command_unknown",
    });
    return true;
  }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 3e. Operator admin commands (ADMIN_PHONE, 1:1 only).
    // Runs BEFORE injection deflect so that Yaron can freely test/probe
    // from his operator phone without getting deflected. Short-circuits
    // only when the message is a recognized /command; falls through for
    // normal Hebrew/English user messages Yaron might send.
    if (
      message.chatType === "direct" &&
      (message.senderPhone === ADMIN_PHONE || message.senderPhone === ADMIN_PHONE.replace("+", ""))
    ) {
      const handled = await handleAdminCommand(message, provider);
      if (handled) {
        await logMessage(message, "admin_command");
        return new Response("OK", { status: 200 });
      }
    }

    // 3f. Identity probe / prompt-injection deflect (1:1 + groups, non-operator).
    // Deterministic reply, zero LLM calls. Runs BEFORE Haiku classification and
    // BEFORE the 1:1 unknown-sender waitlist redirect — we want prompt injection
    // deflected with Sheli voice, not waitlist-redirected.
    if (
      message.senderPhone !== ADMIN_PHONE &&
      message.senderPhone !== ADMIN_PHONE.replace("+", "") &&
    ...
    try {
      ...
      if (staleIds.length > 0) {
        const { error: cancelErr } = await supabase
          ...
          .in("id", staleIds);
        if (cancelErr) {
          console.error(`${logPrefix} [Bulk] Soft-cancel error:`, cancelErr);
        } else {
          console.log(`${logPrefix} [Bulk] Soft-cancelled ${staleIds.length} pending reminders in ${groupId}`);
        }
      } else {
        console.log(`${logPrefix} [Bulk] No pending rows in last 2h for ${groupId}`);
      }
    } catch (bulkErr) {
      console.error(`${logPrefix} [Bulk] Exception:`, bulkErr);
    }
  }
